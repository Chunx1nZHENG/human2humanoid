...
from legged_gym.envs import H1_2_Env
...
def train(cfg_hydra: DictConfig) -> None:
    ...
    env = H1_2_Env(
        num_envs=cfg_hydra.num_envs, env_cfg=cfg_hydra, obs_cfg=cfg_hydra, reward_cfg=cfg_hydra, command_cfg=cfg_hydra
    )

    ppo_runner = OnPolicyRunner(env, cfg_hydra.train, log_dir, device="cuda:0")
